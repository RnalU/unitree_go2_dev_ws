...
namespace {
const char *kNodeName = "go2_udp_bridge_host_cpp";
}

class HostBridgeCppNode : public rclcpp::Node {
public:
  HostBridgeCppNode()
      : rclcpp::Node(kNodeName) {
    const auto default_config = ament_index_cpp::get_package_share_directory("go2_udp_wifi_bridge_host_cpp") +
                               std::string("/config/host_bridge_cpp.example.yaml");
    ...
    bridge_core_->start();
    RCLCPP_INFO(this->get_logger(), "Host C++ UDP bridge started with config %s", config_file.c_str());
  }

  ~HostBridgeCppNode() override { bridge_core_.reset(); }
...
int main(int argc, char **argv) {
  ...
  try {
    auto node = std::make_shared<HostBridgeCppNode>();
    rclcpp::spin(node);
